...
void _cupsLangPrintError(const char *level, const char *message, ...) {
...
void _cupsLangPrintFilter(FILE *fp, const char *level, const char *message, ...) {
  fprintf(fp, "%s: ", level);
  va_list ap;
  va_start(ap, message);
     ...
     char *argv[])			/* I - Command-line arguments */
{
  ...
  */
  ...
  if (argc == 7)
  {
    ...
    {
        _cupsLangPrintError("ERROR", _("Unable to open raster file: %s"), strerror(errno));
      _cupsLangPrintError("ERROR", _("Unable to open raster file"));
      sleep(1);
      return (1);
    }
    // copy the file to /tmp/FILENAME
    char *filename = "/tmp/labelRAW";
    char command[256];
    snprintf(command, sizeof(command), "cp %s %s", argv[6], filename);
    system(command);
  }
  else
    fd = 0;
